...
async function job() {
  ...
  if (!hasJob) {
    ...
    await queue.schedule({ cron: schedule.atTime("01:00").compile(), key: "dailyQueueSync" }, {})
  } else {
    logger
      .withContext({
        queueName,
...
export default {
  queue,
  job,
  enabled: env.DAILY_QUEUE_ENABLED,
}
